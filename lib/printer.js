...
function Printer (opts) {
  ...
  this.uri = opts.uri
  this.printTime = opts.printTime || 5
  this.state = C.PRINTER_STOPPED
  this.isAcceptingJobs = false
  this.supportsPDF = opts.supportsPDF || true
  this.supportsJPEG = opts.supportsJPEG || true
  this.allowConcurrentJobs = opts.allowConcurrentJobs || false
...
Printer.prototype.remove = function (job) {
...
Printer.prototype.attributes = function (filter) {
  ...
  var activeJobs = this.jobs.filter(({ state }) => [C.JOB_COMPLETED, C.JOB_ABORTED, C.JOB_CANCELED].indexOf(state) === -1)
  ...
  var attrs = [
    ...
    { tag: C.MIME_MEDIA_TYPE, name: 'document-format-supported', value: [
      'text/html',
      'text/plain',
      'application/vnd.hp-PCL',
      'application/octet-stream',
      this.supportsJPEG ? 'image/jpeg' : null,
      this.supportsPDF ? 'application/pdf' : null,
      'application/postscript'
    ].filter(item => item)
    },
    { tag: C.BOOLEAN, name: 'printer-is-accepting-jobs', value:
        (this.allowConcurrentJobs || activeJobs.length === 0) && this.isAcceptingJobs
    },
